...
st.markdown(
    """
<style>
.block-container { padding: 0.55rem 0.7rem 4rem 0.7rem; max-width: 430px; }
div.stButton > button {
  width: 100%;
  min-height: 3rem;
  font-size: 1.05rem;
...
def page_lab():
    ...
    if sid == 1:
        ...
        st.success("Legal") if spacing >= 3 else st.error("Too close")
    elif sid == 7:
...
nav = st.radio("Menu", ["Hub", "Stage", "Lab", "Atlas"], horizontal=True, label_visibility="collapsed")
if nav == "Hub":
    page_hub()
elif nav == "Lab":
    page_lab()
