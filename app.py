"""SkyQuest Lite — Android Chrome / small screens. Streamlit only."""
...
def _read_json(name: str, embedded: str):
    for folder in (HERE, Path.cwd(), HERE / "data"):
        path = folder / name
        if path.is_file():
...
def true_plan(stage: dict) -> dict:
    ...
    gs = max(40.0, ac["cruise_kt"] - 0.7 * stage["wind_kt"])
    dist = total_nm(stage)
    hours = dist / gs
    if stage["id"] == 9:
...
st.markdown(
    """
<style>
.block-container { padding: 0.55rem 0.7rem 4rem 0.7rem; max-width: 480px; }
div.stButton > button {
  width: 100%;
  min-height: 3rem;
  font-size: 1.05rem;
...
def new_flight(stage: dict) -> dict:
    ...
    try:
        fuel = float(fuel_plan)
    except (TypeError, ValueError):
        fuel = truth["fuel_gal"]
    fuel = min(ac["fuel_cap_gal"], max(ac["reserve_gal"], fuel))
    return {
        "nm_done": 0.0,
        ...
        "clock_min": 0.0,
        "alts": [800 if stage["id"] == 1 else 4000],
        "seen": [],
        "density": False,
    }


def fire(stage: dict, fl: dict):
    for i, ev in enumerate(stage["events"]):
        ...
        if ev["type"] == "callout":
            ...
            fl["card"] = city
            icao = ev.get("city")
            if icao and icao not in st.session_state.pins:
                st.session_state.pins.append(icao)
        elif ev["type"] == "weather":
            if "wind_add" in ev:
                fl["wind"] += ev["wind_add"]
            if "wind_set" in ev:
                fl["wind"] = ev["wind_set"]
            fl["log"] = ev.get("text", "Wind change.")
        elif ev["type"] == "emergency":
            fl["burn_mult"] = ev.get("burn_mult", fl["burn_mult"])
            fl["log"] = ev.get("text", "Emergency.")
        elif ev["type"] == "atc":
            fl["log"] = ev.get("text", "ATC.")
            if not fl.get("slowed_for_atc"):
                fl["spacing_ok"] = False
        elif ev["type"] == "physics":
            fl["density"] = True
            fl["log"] = ev.get("text", "Thin air.")
        else:
            fl["log"] = ev.get("text", ev["type"])
...
def tick(stage: dict, fl: dict) -> dict:
    ...
    pitch = fl["pitch"]
    tas = ac["cruise_kt"] * (0.35 + 0.75 * fl["throttle"] / 100.0)
    if fl.get("density"):
        tas *= 0.9
    ...
    if stall:
        ...
        fl["log"] = "STALL — nose down, power up."
    ...
    if fl["alt"] <= 0 and fl["nm_done"] < total_nm(stage) * 0.95:
        fl["alive"] = False
        fl["log"] = "Terrain. Flight over."
        return fl
    gs = max(35.0, tas - 0.7 * fl["wind"])
    fl["tas"] = tas
    ...
    if fl["in_hold"]:
        ...
        if fl["fuel"] <= 0:
            fl["alive"] = False
            fl["log"] = "Fuel gone in the hold."
        if fl["hold_min"] >= stage.get("hold_min", 40):
            fl["hold_survived"] = True
            fl["arrived"] = True
            fl["in_hold"] = False
            fl["log"] = "Hold complete."
        fire(stage, fl)
        fl["alts"].append(fl["alt"])
        return fl
    fl["nm_done"] += gs * dt_h
    fl["fuel"] -= used
    fl["alts"].append(fl["alt"])
...
def score_flight(stage: dict, fl: dict) -> dict:
    ...
    if fl["fuel"] < ac["reserve_gal"] and stage["id"] >= 3:
        passed = False
        notes.append("Below reserve")
...
def close_flight(stage: dict, fl: dict):
    res = score_flight(stage, fl)
    st.session_state.last_result = res
    if res["pass"]:
        st.session_state.hours += res["hours"]
        if st.session_state.unlocked == stage["id"] and stage["id"] < 10:
            st.session_state.unlocked = stage["id"] + 1
        st.session_state.logbook.append(
            {"stage": stage["id"], "title": stage["title"], "stars": res["stars"]}
        )
    st.session_state.page = "debrief"
    st.session_state.pending_nav = "Play"


# ----- pages -----

def page_hub():
    st.title("SkyQuest Lite")
    st.caption("Android Chrome · South Asia + Europe")
    st.write(f"**{rank_for(st.session_state.unlocked)}** · stage {st.session_state.unlocked}/10 · {st.session_state.hours:.1f} h")
    for s in STAGES:
        locked = s["id"] > st.session_state.unlocked
        label = f"{s['id']:02d}  {s['title']}"
        if locked:
            st.button(label + "  lock", key=f"l{s['id']}", disabled=True)
        else:
            if st.button(label, key=f"o{s['id']}"):
                st.session_state.active = s["id"]
                st.session_state.player_plan = {}
                st.session_state.flight = None
                st.session_state.page = "brief"
                st.session_state.pending_nav = "Play"
                st.rerun()
...
def page_brief():
    ...
    st.caption(f"Stage {s['id']} / 10")
    st.title(s["title"])
    st.write(s["hook"])
    st.write(s["history"])
    st.write("**Learn:** " + s["teach"])
    st.info(s["formula"])
    names = " → ".join(CITIES[c]["name"].split("/")[0] for c in s["route"])
    st.write("**Route:** " + names)
    st.write(s["note_nm"])
    st.write(f"**Ship:** {ac['name']} · {ac['cruise_kt']} kt · burn {ac['burn_gph']} gph")
    st.write(f"**Wind:** {s['wind_kt']} kt")
    st.write("**Adventure:** " + s["adventure"])
    if st.button("Open this stage lab"):
        st.session_state.page = "lab"
        st.session_state.pending_nav = "Lab"
        st.rerun()
    if st.button("Write the plan"):
        st.session_state.page = "plan"
        st.session_state.pending_nav = "Play"
        st.rerun()
    if st.button("Hub"):
        st.session_state.page = "hub"
        st.session_state.pending_nav = "Hub"
        st.rerun()
...
def page_plan():
    ...
    if "legs_sum_nm" in fields:
        plan["legs_sum_nm"] = st.number_input("Sum of legs (NM)", min_value=0.0, step=1.0)
    if "gs_kt" in fields:
        plan["gs_kt"] = st.number_input("Groundspeed (kt)", min_value=0.0, step=1.0)
    if "ete_min" in fields:
        plan["ete_min"] = st.number_input("ETE (minutes)", min_value=0.0, step=1.0)
    if "fuel_gal" in fields:
        plan["fuel_gal"] = st.number_input("Fuel to load (gal)", min_value=0.0, max_value=float(ac["fuel_cap_gal"]), step=1.0)
    if "endurance_h" in fields:
        plan["endurance_h"] = st.number_input("Endurance (hours)", min_value=0.0, step=0.1)
    if "pnr_min" in fields:
        plan["pnr_min"] = st.number_input("PNR minute in hold", min_value=0.0, step=1.0)
    if "spacing_action" in fields:
        plan["spacing_action"] = st.radio(
            "Traffic 8 NM ahead, same speed. To keep 3 NM:",
            ["Do nothing", "Slow about 20 kt", "Speed up"],
        )
        st.session_state.pre_slow = plan["spacing_action"] == "Slow about 20 kt"
    if "divert" in fields:
        plan["divert"] = st.radio("Burn +15% after Naples", ["Press to Athens", "Use Brindisi"])
    if "arrival_local" in fields:
        plan["arrival_local"] = st.text_input("Dum Dum clock (IST)", placeholder="19:40")
...
def page_fly():
    ...
    if fl is None:
        st.session_state.page = "plan"
        st.rerun()
    st.caption(f"LIVE · {s['title']}")
    dist = total_nm(s)
    st.progress(min(1.0, fl["nm_done"] / max(0.1, dist)))
    a, b = st.columns(2)
    a.metric("TAS", f"{fl['tas']:.0f} kt")
    b.metric("ALT", f"{fl['alt']:.0f} ft")
    c, d = st.columns(2)
    c.metric("FUEL", f"{max(0, fl['fuel']):.1f} gal")
    d.metric("DONE", f"{fl['nm_done']:.0f}/{dist:.0f} NM")
    st.caption("Top-down")
    bits = []
    acc = 0.0
    for i, icao in enumerate(s["route"]):
        if i:
            acc += s["legs_nm"][i - 1]
        name = CITIES.get(icao, {}).get("name", icao).split("/")[0]
        mark = "▶" if abs(acc - fl["nm_done"]) < max(8.0, dist * 0.04) else "•"
        bits.append(f"{mark} {name}")
    st.write("  ".join(bits))
    st.caption("Side view (altitude)")
    st.line_chart(fl["alts"][-40:], height=160)
    fl["throttle"] = st.slider("Throttle", 0, 100, int(fl["throttle"]), key="lite_thr")
    fl["pitch"] = st.slider("Pitch", -8, 16, int(fl["pitch"]), key="lite_pitch")
    dead = (not fl["alive"]) or fl["arrived"]
    if st.button("ADVANCE", disabled=dead):
        st.session_state.flight = tick(s, fl)
        st.rerun()
    if s["id"] in (6, 10) and st.button("Tell ATC: slowing"):
        fl["slowed_for_atc"] = True
        fl["spacing_ok"] = True
        fl["throttle"] = max(40, fl["throttle"] - 15)
        fl["log"] = "Slowed. Spacing should open."
        st.rerun()
    if s["id"] == 9 and fl.get("in_hold"):
        if st.button("TURN BACK"):
            ...
            fl["arrived"] = True
            fl["hold_survived"] = True
            fl["log"] = "Turned back to Shannon."
            st.rerun()
    if fl.get("card"):
        city = fl["card"]
        st.info(f"{city.get('name','')}\n\n{city.get('line','')}\n\n{city.get('why','')}")
    if fl.get("log"):
        st.warning(fl["log"]) if any(w in fl["log"].upper() for w in ("STALL", "FUEL", "TERRAIN", "ROUGH")) else st.success(fl["log"])
    if dead and st.button("Debrief"):
        close_flight(s, fl)
        st.rerun()
...
def page_debrief():
    ...
    st.title("Debrief")
    st.write(s["title"])
    st.write("★" * res.get("stars", 0) + "☆" * (3 - res.get("stars", 0)))
    if res.get("pass"):
        st.success("Stage complete." if s["id"] < 10 else "Long hop complete.")
    else:
        st.warning("Not yet. Fly it again.")
    st.write(res.get("notes", ""))
    truth = res.get("truth", true_plan(s))
    plan = st.session_state.player_plan
    st.write(f"Your ETE {plan.get('ete_min', '—')} min · book {truth['ete_min']} min")
    st.write(f"Your fuel {plan.get('fuel_gal', '—')} · book {truth['fuel_gal']} gal")
    if st.button("Hub"):
        st.session_state.page = "hub"
        st.session_state.pending_nav = "Hub"
        st.rerun()
    if st.button("Fly again"):
        st.session_state.flight = None
        st.session_state.page = "plan"
        st.session_state.pending_nav = "Play"
        st.rerun()
    if st.button("This stage lab"):
        st.session_state.page = "lab"
        st.session_state.pending_nav = "Lab"
        st.rerun()
...
def page_lab():
    st.title("Flight Lab")
    open_ids = [s["id"] for s in STAGES if s["id"] <= st.session_state.unlocked]
    labels = {s["id"]: f"{s['id']} {s['title']}" for s in STAGES}
    choice = st.selectbox(
        "Stage",
        open_ids,
        index=open_ids.index(st.session_state.active) if st.session_state.active in open_ids else 0,
        format_func=lambda i: labels[i],
    )
    s = stage_by_id(int(choice))
    ac = AIRCRAFT[s["aircraft"]]
    truth = true_plan(s)
    st.write(s["teach"])
    st.info(s["formula"])
    sid = s["id"]
    if sid == 1:
        thrust = st.slider("Thrust", 0, 100, 60)
        mass = st.slider("Mass", 20, 100, 50)
        flaps = st.slider("Flaps", 0, 40, 0)
        speed = st.slider("Speed", 40, 120, 85)
        lift = 0.4 * speed + flaps * 0.8
        weight = mass * 1.1
        drag = 18 + flaps * 0.5
        st.bar_chart({"Lift": lift, "Weight": weight, "Thrust": float(thrust), "Drag": drag})
        st.write("Climb if Lift > Weight. Speed up if Thrust > Drag.")
    elif sid == 2:
        speed = st.slider("Speed kt", 30, 90, int(ac["cruise_kt"]))
        st.metric("ETE min", f"{(5.2 / speed) * 60:.1f}")
        st.write("5.2 NM ÷ speed × 60")
    elif sid == 3:
        tas = st.slider("TAS kt", 60, 120, int(ac["cruise_kt"]))
        wind = st.slider("Headwind kt", 0, 40, int(s["wind_kt"]))
        gs = max(40.0, tas - wind)
        hours = truth["dist_nm"] / gs
        st.metric("GS", f"{gs:.0f} kt")
        st.metric("Fuel gal", f"{ac['burn_gph'] * hours + ac['reserve_gal']:.1f}")
    elif sid == 4:
        gs = st.slider("GS kt", 60, 140, int(truth["gs_kt"]))
        total = sum(s["legs_nm"])
        st.metric("Total NM", f"{total:.1f}")
        st.metric("ETE min", f"{(total / gs) * 60:.0f}")
    elif sid == 5:
        elev = st.slider("Elevation ft", 0, 8000, 4390)
        pitch = st.slider("Nose up", 0, 16, 4)
        thin = 1.0 - elev / 40000
        lift = 90 * thin * (1 + pitch * 0.04)
        st.metric("Lift vs 100 weight", f"{lift:.0f}")
        if pitch > 10 and thin < 0.9:
            st.warning("Stall. Lower the nose.")
    elif sid == 6:
        lead = st.slider("Lead NM", 4, 30, 8)
        you = st.slider("You NM", 4, 30, 8)
        ls = st.slider("Lead kt", 80, 180, 140)
        ys = st.slider("You kt", 80, 180, 140)
        gap = you / ys * 60 - lead / ls * 60
        spacing = abs(gap) / 60 * (ls if gap > 0 else ys)
        st.metric("Spacing NM", f"{spacing:.1f}")
        st.success("Legal") if spacing >= 3 else st.warning("Too close — slow down.")
    elif sid == 7:
        tas = st.slider("TAS", 80, 180, int(ac["cruise_kt"]))
        w = st.slider("Headwind this leg", 0, 40, 20)
        st.metric("GS", f"{max(40, tas - w):.0f} kt")
    elif sid == 8:
        burn = st.slider("Burn gph", 50, 120, int(ac["burn_gph"]))
        fuel = st.slider("Fuel gal", 80, 400, 300)
        st.metric("Hours left", f"{max(0, (fuel - ac['reserve_gal']) / burn):.2f}")
    elif sid == 9:
        hold = st.slider("Minutes held", 0, 40, 18)
        st.write("If burn jumped at minute 18, turn back.")
        if hold >= 18:
            st.warning("Turn back is the clean number.")
    else:
        gs = st.slider("GS kt", 300, 500, 420)
        hours = truth["dist_nm"] / gs
        st.metric("Flight hours", f"{hours:.1f}")
        st.write("London UTC+0. Dum Dum IST = UTC+5:30.")
    if st.button("Back to brief"):
        st.session_state.active = s["id"]
        st.session_state.page = "brief"
        st.session_state.pending_nav = "Play"
...
def page_atlas():
    ...
    if st.session_state.logbook:
        for row in st.session_state.logbook:
            st.write(f"Stage {row['stage']} {row['title']}  {'★' * row['stars']}")
    else:
        st.caption("No flights closed.")
    if st.button("Hub"):
        st.session_state.page = "hub"
        st.session_state.pending_nav = "Hub"
        st.rerun()
